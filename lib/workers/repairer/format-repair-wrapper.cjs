...
async function detectFormatterCommand(packageManager, repositoryPath) {
	try {
		const pkgRaw = await readFile(`${repositoryPath}/package.json`, "utf8");
...
async function detectPackageManager(repositoryPath) {
	try {
		await readFile(`${repositoryPath}/pnpm-lock.yaml`, "utf8");
		return "pnpm";
	} catch {
		// ignore
	}
	...
	try {
		await readFile(`${repositoryPath}/yarn.lock`, "utf8");
		return "yarn";
	} catch {
		// ignore
	}
...
(async () => {
	...
	try {
		...
		const formatter = await detectFormatterCommand(
			packageManager,
			repositoryPath,
		);
